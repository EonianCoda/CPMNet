...
def get_args():
    ...
    # Data
    parser.add_argument('--data_set_class', type=str, default='dataload.dataset_oversample', help='data set class')
    parser.add_argument('--max_oversample_times', type=int, default=3, help='max oversample times')
    parser.add_argument('--oversample_n_clusters', type=int, default=10, help='number of clusters for oversample')
...
def get_train_dataloder(args, blank_side=0) -> DataLoader:
    ...
    train_dataset = train_set_class(series_list_path = args.train_set, crop_fn = crop_fn_train, image_spacing=IMAGE_SPACING, transform_post = train_transform, 
                                 ...
                                 max_oversample_times=args.max_oversample_times, oversample_n_clusters=args.oversample_n_clusters)
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, end_epoch + 1):
        ...
        if getattr(train_loader.dataset, 'oversample', None) is not None:
            train_loader.dataset.oversample()
