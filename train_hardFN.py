...
from dataload.collate import train_hardFN_collate_fn, infer_collate_fn
...
### logic ###
from logic.train_hardFN import train
...
def get_args():
    ...
    # Data
    parser.add_argument('--data_set_class', type=str, default='dataload.dataset_hardFN', help='data set class')
...
def prepare_training(args, device, num_training_steps) -> Tuple[int, Any, AdamW, GradualWarmupScheduler, Any]:
    ...
    return start_epoch, model, optimizer, scheduler_warm, ema, val_detection_postprocess, test_detection_postprocess, detection_loss
...
def get_train_dataloder(args, blank_side=0) -> DataLoader:
    ...
    from dataload.crop_hardFN import InstanceCrop
    ...
    train_loader = DataLoader(train_dataset, 
                              ...
                              shuffle=True,
                              collate_fn=train_hardFN_collate_fn,
...
if __name__ == '__main__':
    ...
    start_epoch, model, optimizer, scheduler_warm, ema, val_det_postprocess, test_det_postprocess, detection_loss = prepare_training(args, device, len(train_loader) // args.iters_to_accumulate)
    ...
    hardFN_label_folder = os.path.join(exp_folder, 'labels')
    os.makedirs(hardFN_label_folder, exist_ok=True)
    for epoch in range(start_epoch, end_epoch + 1):
        ...
        with open(os.path.join(hardFN_label_folder, f'epoch_{epoch}.pkl'), 'wb') as f:
            pickle.dump(train_loader.dataset.labels, f)
        # Write metrics
