...
class InstanceCrop(object):
    ...
    def __call__(self, sample, image_spacing: np.ndarray):
        images = sample['image'].astype('float32')
        ...
        for i in range(images.shape[0]):
            image_itk = sitk.GetImageFromArray(images[i])
            image_itks.append(image_itk)
        ...
        for i in index:
            ...
            CT_crops.append(np.stack(image_crops, axis=0))
