...
class TrainDataset(Dataset):
    ...
    def __init__(self, series_list_path: str, image_spacing: List[float], transform_post=None, crop_fn=None, 
                 use_bg=False, min_d=0, min_size: int = 0, norm_method='scale', mmap_mode=None, max_oversample_times = 3, oversample_n_clusters = 10):
        self.labels = []
        self.dicom_paths = []
        self.series_names = []
        self.series_folders = []
        ...
        nodule_typer = NoduleTyper(NODULE_TYPE_DIAMETERS, image_spacing=image_spacing)
        noduleTypes2Idx = {noduleType: idx for idx, noduleType in enumerate(NODULE_TYPE_DIAMETERS.keys())}
        series_nodule_type_feats = []
        ...
        for folder, series_name in self.series_infos:
            ...
            nodule_types = [nodule_typer.get_nodule_type_by_seg_size(size) for size in label[NODULE_SIZE]]
            feat = [0] * len(NODULE_TYPE_DIAMETERS)
            for t in nodule_types:
                feat[noduleTypes2Idx[t]] += 1
            series_nodule_type_feats.append(feat)
        
        ## Cluster the nodule type features
        # Convert the list to numpy array
        series_nodule_type_feats = np.array(series_nodule_type_feats)
        # Normalize the nodule type features
        series_nodule_type_feats = (series_nodule_type_feats - np.mean(series_nodule_type_feats, axis=0)) / np.std(series_nodule_type_feats, axis=0)
        
        self.max_oversample_times = max_oversample_times
        self.n_clusters = oversample_n_clusters
        kmeans = KMeans(n_clusters = self.n_clusters, random_state = 0, n_init = 'auto')
        kmeans.fit(series_nodule_type_feats)
    ...
    def oversample(self):
        # Oversample clusters with fewer samples
        ...
        for i in cluster_indices:
            ...
            if num_sample_in_cluster < (avg_sample_num // self.max_oversample_times): # Oversample
                ...
                remaining = sample_num % num_sample_in_cluster
                new_indices.extend(np.random.choice(sample_indices, remaining, replace=False).tolist())
            else:
                new_indices.extend(sample_indices)
