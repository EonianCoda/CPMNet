...
class InstanceCrop(object):
    ...
    def __call__(self, sample, lobe, image_spacing: np.ndarray, use_gt_crop=False):
        ...
        if has_gt and len(gt_all_loc) != 0:
        ...
        elif has_gt and len(gt_instance_loc) > 0 and use_gt_crop:
            if self.rand_trans is not None:
        ...
        if has_gt and len(gt_instance_loc) > 0:
        ...
        if not use_gt_crop:
            ...
            if not has_gt:
                raise ValueError('No GT provided')
