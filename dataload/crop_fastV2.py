...
class InstanceCrop(object):
    ...
    def __call__(self, sample, image_spacing: np.ndarray):
        ...
        sample_indices = []
        num_tp = int(self.sample_num * self.tp_ratio)
        if len(instance_loc) > num_tp:
            sample_indices.extend(np.random.choice(range(num_fixed_crop_centers, len(crop_centers)), size=num_tp, replace=False))
        elif len(instance_loc) > 0 and len(instance_loc) <= num_tp:
            sample_indices.extend(list(range(num_fixed_crop_centers, len(crop_centers))))
            
        num_fp = self.sample_num - len(sample_indices)
        if num_fp > 0:
            neg_indices = np.nonzero((max_ious == 0.0)[:num_fixed_crop_centers])[0]
            if len(neg_indices) == 0:
                sample_indices.extend(np.random.choice(range(num_fixed_crop_centers - 1), size=num_fp, replace=False))
            else:
                sample_indices.extend(np.random.choice(neg_indices, size=num_fp, replace=False))
