...
class InstanceCrop(object):
    ...
    def __init__(self, crop_size, rand_trans=None, rand_rot=None, instance_crop=True, overlap_ratio: float=0.25, 
        ...
        self.base_crop_size = np.array(crop_size, dtype=np.int32)
        self.crop_size_candidates = [self.base_crop_size, self.base_crop_size + 32, self.base_crop_size + 16, self.base_crop_size - 16, self.base_crop_size - 32]
        
        self.crop_size = np.array(crop_size, dtype=np.int32)
        self.overlap_ratio = overlap_ratio
        self.overlap_size = np.array(self.crop_size * self.overlap_ratio, dtype=np.int32)
        ...
        if rand_trans == None:
            self.base_rand_trans = None
            self.rand_trans = None
        else:
            self.base_rand_trans = np.array(rand_trans)
    ...
    def change_scale(self):
        ...
        self.crop_size = self.crop_size_candidates[rand_idx]
        self.overlap_size = (self.crop_size * self.overlap_ratio).astype(np.int32)
        self.stride_size = self.crop_size - self.overlap_size
        self.rand_trans = (self.base_rand_trans * (self.crop_size / self.base_crop_size)).astype(np.int32)
