...
class InstanceCrop(object):
    ...
    def __call__(self, sample, image_spacing: np.ndarray):
        ...
        if num_fp > 0:
            neg_indices = np.nonzero((tp_nums == 0)[:num_fixed_crop_centers])[0]
            if len(neg_indices) == 0:
                sample_indices.extend(np.random.choice(range(num_fixed_crop_centers), size=num_fp, replace=False))
            else:
