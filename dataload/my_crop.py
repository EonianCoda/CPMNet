...
class InstanceCrop(object):
    ...
    def __call__(self, sample, image_spacing):
        ...
        for i in range(len(crop_centers)):
            ...
            O = np.clip(O, a_min=0, a_max=None)
            O = np.where(O + crop_size > shape, shape - crop_size, O)
            # if origin[i] + crop_size[i] > shape[i]:
            #     all_loc_crops[i] = all_loc_crops[i] + (shape[i] - crop_size[i] - origin[i])
            #     origin[i] = shape[i] - crop_size[i]
            # if origin[i] < 0:
            #     all_loc_crops[i] = all_loc_crops[i] + (-origin[i])
            #     origin[i] = 0
                    
            all_loc_crop = all_loc - O[np.newaxis, :]
            all_loc_crop = np.array(all_loc_crop)
            in_idx = []
            for j in range(all_loc_crop.shape[0]):
                if (all_loc_crop[j] < crop_size).all() and (
            ...
            origin_crops.append(O)
            space_crops.append(self.spacing)
        ...
        # loc_crops = [all_loc_crops[i] for i in index]
        # rad_crops = [all_rad_crops[i] for i in index]
        # cls_crops = [all_cls_crops[i] for i in index]
        
        
        for index_i, i in enumerate(index):
            origin = origin_crops[i]
            space = space_crops[i]
            ...
            print('origin = ', origin)
            print('cls = ', all_cls_crops[index_i])
            image_crop = image[int(origin[0]): int(origin[0] + crop_size[0]),
                                int(origin[1]): int(origin[1] + crop_size[1]),
                                int(origin[2]): int(origin[2] + crop_size[2])]
