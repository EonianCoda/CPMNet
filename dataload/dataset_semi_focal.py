...
class UnLabeledDataset(Dataset):
    def __init__(self, series_list_path: str, image_spacing: List[float], strong_aug = None, crop_fn=None, 
                 use_bg=False, min_d=0, min_size: int = 0, norm_method='scale', mmap_mode=None, num_aug=3):
        self.series_list_path = series_list_path
        self.norm_method = norm_method
        self.image_spacing = np.array(image_spacing, dtype=np.float32) # (z, y, x)
        self.min_d = int(min_d)
    ...
    def __getitem__(self, idx):
        ...
        aug_samples = [[] for _ in range(self.num_aug)] 
        ...
        for i in range(len(samples)):
            ...
            for j in range(self.num_aug):
                aug_samples[j].append(self.strong_aug(copy.deepcopy(sample)))
        
        random_samples = dict()
        for i, aug_samples in enumerate(aug_samples):
            random_samples[f'aug_{i}'] = aug_samples
