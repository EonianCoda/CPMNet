...
from .utils import ALL_HARD_FP_RAD, ALL_HARD_FP_LOC, ALL_CLS, ALL_RAD, ALL_LOC
...
class InstanceCrop(object):
    ...
    def __init__(self, crop_size, overlap_ratio: float = 0.25, rand_trans=None, rand_rot=None, instance_crop=True, 
        ...
        Arguments:
            ...
            fp_ratio: sampling rate for a patch containing no leision
    ...
    def __call__(self, sample, image_spacing: np.ndarray):
        ...
        if ALL_HARD_FP_LOC in sample:
            hard_fp_all_loc = sample[ALL_HARD_FP_LOC]
            hard_fp_all_rad = sample[ALL_HARD_FP_RAD]
        else:
        ...
        for i in range(len(crop_centers)):
            ...
            if len(hard_fp_all_loc) == 0:
                ...
                hard_fp_num = np.sum((hard_fp_all_loc > bb_min).all(axis=1) & (hard_fp_all_loc < bb_max).all(axis=1))
            hard_fp_nums.append(hard_fp_num)
        ...
        if tp_and_hard_fp_idx.sum() > 0:
            ...
            tp_and_hard_fp_prob = 0
            
        # TP
        if tp_idx.sum() > 0:
            tp_prob = self.tp_ratio / tp_idx.sum()
        else:
            tp_prob = 0
        ...
        p[tp_and_hard_fp_idx] = tp_and_hard_fp_prob
        p[neg_idx] = max(1. - p.sum(), 1e-3) / neg_idx.sum() if neg_idx.sum() > 0 else 0
        p = p / p.sum()
