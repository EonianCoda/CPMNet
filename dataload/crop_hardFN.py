...
class InstanceCrop(object):
    ...
    def __call__(self, sample, image_spacing: np.ndarray):
        ...
        if tp_idx.sum() > 0:
            ...
            hard_tp_probs = (1 - tp_probs[tp_idx] + 1e-3) ** 2
            # Sum of hard_tp_probs is self.tp_ratio
            tp_pos = (hard_tp_probs / hard_tp_probs.sum()) * self.tp_ratio
        else:
            tp_pos = 0
        ...
        for sample_i in sample_indices:
            ...
            sample['rad'] = rad
            sample['cls'] = cls
            sample['nodule_indices'] = np.array(np.where(in_idx)[0])
            sample['spacing'] = image_spacing.copy()
            sample['series_name'] = series_name
