...
def get_position(image):
    ...
    xx_channel = torch.arange(x_dim).repeat(y_dim, z_dim, 1).transpose(0, 1)
    yy_channel = torch.arange(y_dim).repeat(x_dim, z_dim, 1).transpose(1, 2)
    zz_channel = torch.arange(z_dim).repeat(x_dim, y_dim, 1)
    ...
    yy_channel = yy_channel.transpose(0, 2)
    zz_channel = zz_channel.transpose(0, 2)

    xx_channel = xx_channel.numpy()
    yy_channel = yy_channel.numpy()
    zz_channel = zz_channel.numpy()
    
    position = np.stack([zz_channel, yy_channel, xx_channel], axis=0)
    return position
...
class DetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        split_images, nzhw, image_shape = self.splitcomb.split(image, position)
