...
from .utils import load_series_list, load_image, load_label, load_lobe, gen_lobe_path, ALL_RAD, ALL_LOC, ALL_CLS, gen_dicom_path, gen_label_path, normalize_processed_image, normalize_raw_image
...
class DetDataset(Dataset):
    ...
    """
    def __init__(self, series_list_path: str, image_spacing: List[float], SplitComb, norm_method='scale', out_stride = 4):
        self.series_list_path = series_list_path
        ...
        for folder, series_name in self.series_infos:
            dicom_path = gen_dicom_path(folder, series_name)
            lobe_path = gen_lobe_path(folder, series_name)
            self.dicom_paths.append(dicom_path)
            self.lobe_paths.append(lobe_path)
    ...
    def __getitem__(self, idx):
        ...
        split_images, split_lobes, nzhw, image_shape = self.splitcomb.split(image, lobe, self.out_stride)
