...
class SplitComb():
    def __init__(self, crop_size: List[int]=[64, 128, 128], overlap_size:List[int]=[16, 32, 32], do_padding:bool=True, pad_value:int=0):
        ...
        self.do_padding = False ##TODO: Change this to do_padding

    def split(self, data, position):
        splits = []
        d, h, w = data.shape
        ...
        for iz in range(nz):
            for iy in range(ny):
                for ix in range(nx):
                    ...
                    position_split = position[np.newaxis, :, start_z:end_z, start_y:end_y, start_x:end_x]
                    split = data[np.newaxis, np.newaxis, start_z:end_z, start_y:end_y, start_x:end_x]
                    split = np.concatenate([split, position_split], 1) # [1, 4, crop_z, crop_y, crop_x]
                    splits.append(split)
