...
class UnLabeledDataset(Dataset):
    def __init__(self, series_list_path: str, image_spacing: List[float], strong_aug = None, crop_fn=None, use_bg=False, 
                 min_d=0, min_size: int = 0, norm_method='scale', mmap_mode=None, use_gt_crop=True, n_samples = 5):
        self.series_list_path = series_list_path
        self.norm_method = norm_method
        self.image_spacing = np.array(image_spacing, dtype=np.float32) # (z, y, x)
        self.min_d = int(min_d)
        ...
        for i in range(len(self.dicom_paths)):
            series_name = self.series_names[i]
            label = self.gt_labels[series_name]
            num_nodules = label[ALL_LOC].shape[0]
            self.samples.append([[i, 0, num_nodules]])
    ...
    def shuffle_batches(self):
        random_indices = np.random.permutation(len(self.dicom_paths))
        self.samples = []
        
        sample = []
        cur_sample_nodule_count = 0
        for i in random_indices:
            ...
            start = 0     
            while cur_sample_nodule_count + num_nodules >= self.n_samples:
                remain_num = self.n_samples - cur_sample_nodule_count
                sample.append([i, start, start + remain_num])
                self.samples.append(sample)
                
                start = start + remain_num
                num_nodules = num_nodules - remain_num
                cur_sample_nodule_count = 0
                sample = []
            
            if num_nodules > 0:
                sample.append([i, start, start + num_nodules])
                cur_sample_nodule_count += num_nodules
    ...
    def __getitem__(self, i):
        all_samples = []
        for idx, start_idx, end_idx in self.samples[i]:
            dicom_path = self.dicom_paths[idx]
            ...
            samples[ALL_LOC] = label[ALL_LOC][start_idx: end_idx] # z, y, x
            samples[ALL_RAD] = label[ALL_RAD][start_idx: end_idx] # d, h, w
            samples[ALL_CLS] = label[ALL_CLS][start_idx: end_idx]
