...
class TrainDataset(Dataset):
    ...
    def __init__(self, series_list_path: str, image_spacing: List[float], transform_post=None, crop_fn=None, 
                 use_bg=False, min_d=0, min_size: int = 0, norm_method='scale'):
        self.labels = []
        self.dicom_paths = []
        ...
        for (folder, series_name), dicom_path, series_labels in zip(self.series_infos, self.dicom_paths, self.labels):
            for i in range(len(series_labels[ALL_LOC])):
                label = dict()
                for key in [ALL_RAD, ALL_LOC, ALL_CLS]:
                    label[key] = [series_labels[key][i]]
                self.datas.append((folder, series_name, dicom_path, series_labels, label))
    ...
    def __getitem__(self, idx):
        series_folder, series_name, dicom_path, series_labels, label = self.datas[idx]
        image_spacing = self.image_spacing.copy() # z, y, x
        image = self.load_image(dicom_path) # z, y, x
        
        sample = {}
        sample['image'] = image
        sample['series_labels'] = copy.deepcopy(series_labels)
        sample['target'] = copy.deepcopy(label)
        sample['file_name'] = series_name
        sample = self.crop_fn(sample, image_spacing)
