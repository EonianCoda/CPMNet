...
def get_args():
    ...
    # Model
    parser.add_argument('--model_class', type=str, default='networks.ResNet_3D_CPM.ResNet_3D_CPM_fpn_lessHard_noX4_mutual_learning', help='model class')
...
def prepare_training(args, device, num_training_steps) -> Tuple[int, Any, AdamW, GradualWarmupScheduler, Any]:
    ...
    model2 = Resnet18(norm_type = args.norm_type,
                     head_norm = args.head_norm, 
                     act_type = args.act_type, 
                     se = not args.no_se, 
                     aspp = args.aspp,
    ...
    model2 = model2.to(device=device, memory_format=get_memory_format(getattr(args, 'memory_format', 'channels_first')))
    # build optimizer and scheduler
    params = add_weight_decay(model, args.weight_decay)
    params2 = add_weight_decay(model2, args.weight_decay)
    ...
    if args.apply_ema:
        ...
        ema2 = EMA(model2, momentum = args.ema_momentum, warmup_steps = ema_warmup_steps)
        ema2.register()
    else:
        ema = None
        ema2 = None
    ...
    if args.resume_folder != '':
        ...
        load_states(ckpt_path, device, model, optimizer, scheduler_warm, ema, model2=model2, ema2=ema2)
        ...
        raise ValueError('Not support pretrained model')
        logger.info('Load model from "{}"'.format(args.pretrained_model_path))
        load_states(args.pretrained_model_path, device, model)
        
    return start_epoch, model, model2, optimizer, scheduler_warm, ema, ema2, val_detection_postprocess, test_detection_postprocess
...
if __name__ == '__main__':
    ...
    start_epoch, model, model2, optimizer, scheduler_warm, ema, ema2, val_det_postprocess, test_det_postprocess = prepare_training(args, device, len(train_loader) // args.iters_to_accumulate)
    ...
    for epoch in range(start_epoch, end_epoch + 1):
        ...
        for i in range(epoch):
            ...
            if ((i % args.save_model_interval != 0 or i == 0 or i < args.start_val_epoch) and os.path.exists(ckpt_path)):
                os.remove(ckpt_path)
        save_states(os.path.join(model_save_dir, f'epoch_{epoch}.pth'), model, optimizer, scheduler_warm, ema, model2=model2, ema2=ema2)
