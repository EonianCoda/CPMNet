...
from logic.utils import load_teacher_model, get_memory_format, load_states
...
def load_model(load_path: str):
    ...
    if 'model_structure' in checkpoint:
        ...
        model2 = checkpoint['model_structure']
    else:
...
def prepare_validation(args, device):
    ...
    model, model2 = load_model(args.model_path)
    if args.load_ema:
        ...
        ema2 = EMA(model2)
        ema2.register()
        ema2.load_state_dict(ckpt['ema2_state_dict'])
        ema2.apply_shadow()
    ...
    model2 = model2.to(device = device, memory_format=memory_format)
    return model, model2, detection_postprocess
...
if __name__ == '__main__':
    ...
    for model_path in model_paths:
        ...
        model, model2, detection_postprocess = prepare_validation(args, device)
        init_seed(args.seed)
