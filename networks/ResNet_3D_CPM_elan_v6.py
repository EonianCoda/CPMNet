...
class ELAN_Layer(nn.Module):
    def __init__(self, n_stages: int, in_channels: int, out_channels: int, stride=1, norm_type='batchnorm', act_type='ReLU', se=False, ratio=2):
        ...
        for i in range(n_stages - 1):
            if i == 0:
                setattr(self, f'conv{i}', BasicBlockNew(out_channels // 2, out_channels // ratio, stride=1, norm_type=norm_type, act_type=act_type, se=se))
            else:
                setattr(self, f'conv{i}', BasicBlockNew(out_channels // ratio, out_channels // ratio, stride=1, norm_type=norm_type, act_type=act_type, se=se))
        
        self.trans = ConvBlock(out_channels + (out_channels // ratio) * (n_stages - 1), out_channels, kernel_size=1, act_type=act_type, norm_type=norm_type)
