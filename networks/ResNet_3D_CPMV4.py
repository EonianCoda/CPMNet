...
class ClsRegHead(nn.Module):
    def __init__(self, in_channels, feature_size=96, conv_num=2,
        ...
        # self.conv_s = nn.Sequential(BasicBlockNew(in_channels, feature_size, stride=1, norm_type=norm_type, act_type=act_type, se=True),
        #                             ConvBlock(feature_size, feature_size, 3, norm_type=norm_type, act_type=act_type))
        # self.conv_r = nn.Sequential(BasicBlockNew(in_channels, feature_size, stride=1, norm_type=norm_type, act_type=act_type, se=True),
        #                             ConvBlock(feature_size, feature_size, 3, norm_type=norm_type, act_type=act_type))
        # self.conv_o = nn.Sequential(BasicBlockNew(in_channels, feature_size, stride=1, norm_type=norm_type, act_type=act_type, se=True),
        #                             ConvBlock(feature_size, feature_size, 3, norm_type=norm_type, act_type=act_type))
        
        self.feat_conv = BasicBlockNew(in_channels, feature_size, stride=1, norm_type=norm_type, act_type=act_type, se=True)
    ...
    def forward(self, x):
        ...
        Shape = self.shape_output(feat)
        Offset = self.offset_output(feat)
...
class Resnet18(nn.Module):
    def __init__(self, n_channels=1, n_blocks=[2, 3, 3, 3], n_filters=[64, 96, 128, 160], stem_filters=32,
        ...
        if out_stride == 4:
            # Head
            self.head = ClsRegHead(in_channels=n_filters[1], feature_size=n_filters[1], conv_num=1, norm_type=head_norm, act_type=act_type)
        elif out_stride == 2:
            ...
            # Head
            self.head = ClsRegHead(in_channels=n_filters[0], feature_size=n_filters[0], conv_num=1, norm_type=head_norm, act_type=act_type)
        self._init_weight()
    ...
    def cls_loss(pred: torch.Tensor, target, mask_ignore, alpha = 0.75 , gamma = 2.0, num_neg = 10000, num_hard = 100, ratio = 100, fn_weight = 4.0, fn_threshold = 0.8):
        ...
        for j in range(batch_size):
            ...
            if num_positive_pixels > 0:
                ...
                # hard_FP_index = torch.gt(cls_prob, 0.7) & (record_targets == 0)
                # cls_loss[hard_FP_index == 1] = 2.0 * cls_loss[hard_FP_index == 1]
                if num_neg != -1:
