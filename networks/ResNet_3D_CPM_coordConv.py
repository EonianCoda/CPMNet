...
class AddCoords(nn.Module):
    def __init__(self):
        ...
        self.zz_channel = None
        self.speed_up = False

    def get_xxyyzz(self, input_tensor):
        batch_size, _, z_dim, y_dim, x_dim = input_tensor.size()
        def gen_xx_yy_zz():
            xx_channel = torch.arange(x_dim).repeat(1, y_dim, z_dim, 1).transpose(1, 2)
            yy_channel = torch.arange(y_dim).repeat(1, x_dim, z_dim, 1).transpose(2, 3)
            zz_channel = torch.arange(z_dim).repeat(1, x_dim, y_dim, 1)

            xx_channel = xx_channel / (x_dim - 1)
            yy_channel = yy_channel / (y_dim - 1)
            zz_channel = zz_channel / (z_dim - 1)
            ...
            yy_channel = yy_channel.repeat(batch_size, 1, 1, 1, 1).transpose(2, 4)
            zz_channel = zz_channel.repeat(batch_size, 1, 1, 1, 1).transpose(2, 4)
    
            return xx_channel, yy_channel, zz_channel

        if self.speed_up:
            if self.xx_channel == None:
                self.xx_channel, self.yy_channel, self.zz_channel = gen_xx_yy_zz()
                if input_tensor.is_cuda:
                    self.xx_channel = self.xx_channel.cuda()
                    self.yy_channel = self.yy_channel.cuda()
                    self.zz_channel = self.zz_channel.cuda()
            return self.xx_channel[:batch_size].clone(), self.yy_channel[:batch_size].clone(), self.zz_channel[:batch_size].clone()
        else:
            xx_channel, yy_channel, zz_channel = gen_xx_yy_zz()
            if input_tensor.is_cuda:
    ...
    def forward(self, input_tensor):
        """
        Args:
            input_tensor: shape(batch, channel, z_dim, y_dim, x_dim)
        """
        xx_channel, yy_channel, zz_channel = self.get_xxyyzz(input_tensor)
        ret = torch.cat([
                    input_tensor,
...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, dilation=1, stride=1, groups=1,
                 norm_type='none', act_type='ReLU', coordConv=False):
        super(ConvBlock, self).__init__()

        if coordConv:
            self.addcoords = AddCoords()
            self.conv = nn.Conv3d(in_channels + 3, out_channels, kernel_size=kernel_size, stride=stride, groups=groups,
...
class BasicBlockNew(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, norm_type='batchnorm', act_type='ReLU', se=True, coordConv=False):
        super(BasicBlockNew, self).__init__()

        self.conv1 = ConvBlock(in_channels=in_channels, out_channels=out_channels, stride=stride,
                               act_type=act_type, norm_type=norm_type, coordConv=coordConv)
...
class LayerBasic(nn.Module):
    def __init__(self, n_stages, in_channels, out_channels, stride=1, norm_type='batchnorm', act_type='ReLU', se=False, coordConv=False):
        ...
        for i in range(n_stages):
            ...
            ops.append(BasicBlockNew(input_channel, out_channels, stride=stride, norm_type=norm_type, act_type=act_type, se=se, coordConv=coordConv))
