...
class Resnet18(nn.Module):
    def __init__(self, n_channels=1, n_blocks=[2, 3, 3, 3], n_filters=[64, 96, 128, 160], stem_filters=32,
        ...
        # Head
        self.help_res = LayerBasic(1, n_filters[1], n_filters[1], norm_type=norm_type, act_type=act_type, se=se)
        self.help_head = ClsRegHead(in_channels=n_filters[1], feature_size=n_filters[1], conv_num=3, norm_type=head_norm, act_type=act_type)
    ...
    def forward(self, inputs):
        ...
        if self.training:
            help_x2 = self.help_res(x2)
            help_out = self.help_head(help_x2)
        ...
        if self.training:
            ...
            cls_help_loss, shape_help_loss, offset_help_loss, iou_help_loss = self.detection_loss(help_out, labels, device=self.device)
            
            ratio = 0.8
            cls_loss = ratio * cls_loss + (1 - ratio) * cls_help_loss
            shape_loss = ratio * shape_loss + (1 - ratio) * shape_help_loss
            offset_loss = ratio * offset_loss + (1 - ratio) * offset_help_loss
            iou_loss = ratio * iou_loss + (1 - ratio) * iou_help_loss
