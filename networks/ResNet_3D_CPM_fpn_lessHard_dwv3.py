...
class Resnet18(nn.Module):
    def __init__(self, n_channels=1, n_blocks=[2, 3, 3, 3], n_filters=[64, 96, 128, 160], stem_filters=32,
        ...
        self.fpn = FPN3D(n_filters, feature_size=n_filters[1], norm_type=norm_type, act_type=act_type)
        ...
        #     self.block1_dw_fpn = DownsamplingBlock(n_filters[0], feature_size, pool_type='avg', norm_type=norm_type, act_type='none')
        #     self.block1_x = ConvBlock(feature_size, feature_size, kernel_size=3, norm_type=norm_type, act_type='none')
        #     self.fpn = FPN3D(n_filters[1:], feature_size=feature_size, norm_type=norm_type, act_type=act_type)
        #     # Head
        #     self.head = ClsRegHead(in_channels=n_filters[1], feature_size=feature_size, conv_num=3, norm_type=head_norm, act_type=act_type)
        # elif out_stride == 2:
    ...
    def forward(self, inputs):
        ...
        feat = self.fpn([x1, x2, x3, x4])
        feat = self.avg_pool3d(feat)
        out = self.head(feat)
        ...
        # "decode"
        # x1_dw = self.block1_dw_fpn(x1)
        # feats = feats + x1_dw
        # feats = self.block1_x(feats)
        # out = self.head(feats)
        if self.training:
