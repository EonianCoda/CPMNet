...
class Resnet18(nn.Module):
    def __init__(self, n_channels=1, n_blocks=[2, 3, 3, 3], n_filters=[64, 96, 128, 160], stem_filters=32,
                 ...
                 first_stride=(2, 2, 2), detection_loss_s2=None, detection_loss_s4=None, device=None, out_stride=4):
        super(Resnet18, self).__init__()
        assert len(n_blocks) == 4, 'The length of n_blocks should be 4'
        assert len(n_filters) == 4, 'The length of n_filters should be 4'
        self.detection_loss_s2 = detection_loss_s2
        self.detection_loss_s4 = detection_loss_s4
        self.device = device
        ...
        # Head
        self.head_s2 = ClsRegHead(in_channels=n_filters[0], feature_size=n_filters[0], conv_num=3, norm_type=head_norm, act_type=act_type)
        self.head_s4 = ClsRegHead(in_channels=n_filters[0], feature_size=n_filters[0], conv_num=3, norm_type=head_norm, act_type=act_type)
        self._init_weight()

    def forward(self, inputs):
        if self.training and self.detection_loss_s2 != None:
            x, labels = inputs
        else:
            x = inputs
        "input encode"
        ...
        feat_s2, feat_s3 = self.fpn([x1, x2, x3])
            
        "decode"
        out_s2 = self.head_s2(feat_s2)
        out_s4 = self.head_s4(feat_s3)
        if self.training and self.detection_loss_s2 != None:
            ...
            offset_loss = (offset_loss_s2 + offset_loss_s4) / 2
            iou_loss = (iou_loss_s2 + iou_loss_s4) / 2
            return cls_pos_loss, cls_neg_loss, shape_loss, offset_loss, iou_loss
        
        return out_s2, out_s4
    ...
    def _init_weight(self):
        ...
        prior = 0.01
        nn.init.constant_(self.head_s2.cls_output.weight, 0)
        nn.init.constant_(self.head_s2.cls_output.bias, -math.log((1.0 - prior) / prior))

        nn.init.constant_(self.head_s2.shape_output.weight, 0)
        nn.init.constant_(self.head_s2.shape_output.bias, 0.5)

        nn.init.constant_(self.head_s2.offset_output.weight, 0)
        nn.init.constant_(self.head_s2.offset_output.bias, 0.05)
        
        nn.init.constant_(self.head_s4.cls_output.weight, 0)
        nn.init.constant_(self.head_s4.cls_output.bias, -math.log((1.0 - prior) / prior))
        
        nn.init.constant_(self.head_s4.shape_output.weight, 0)
        nn.init.constant_(self.head_s4.shape_output.bias, 0.5)
        
        nn.init.constant_(self.head_s4.offset_output.weight, 0)
        nn.init.constant_(self.head_s4.offset_output.bias, 0.05)
