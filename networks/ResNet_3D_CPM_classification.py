...
class ClsRegHead(nn.Module):
    def __init__(self, in_channels, feature_size=96, conv_num=2,
        ...
        self.cls_output = nn.Conv3d(feature_size, 5, kernel_size=3, padding=1) # background + 4 classes
    def forward(self, x):
...
class Resnet18(nn.Module):
    ...
    def _init_weight(self):
        ...
        prior = 0.01
        nn.init.constant_(self.head.object_output.weight, 0)
        nn.init.constant_(self.head.object_output.bias, -math.log((1.0 - prior) / prior))
...
class DetectionLoss(nn.Module):
    ...
    @staticmethod  
    def cls_loss(pred: torch.Tensor, target: torch.Tensor, obj_target, mask_ignore, num_neg = 10000, num_hard = 100, ratio = 100):
    ...
    @staticmethod  
    def obj_loss(pred: torch.Tensor, target, mask_ignore, alpha = 0.75 , gamma = 2.0, num_neg = 10000, num_hard = 100, ratio = 100, fn_weight = 4.0, fn_threshold = 0.8):
        """
    ...
    def get_pos_target(annotations: torch.Tensor,
        ...
        bs, num_annots, num_points = mask_pos.size()
        cls_label = annotations[:, :, 9:14] # shape = (b, num_annotations, 5)
        
        target_cls = torch.ones(bs, num_annots, num_points, 5, dtype=torch.float32, device=mask_pos.device) * torch.clamp(mask_pos, 0, 1).unsqueeze(-1)
        target_cls = target_cls * cls_label.unsqueeze(-2) # shape = (b, num_annotations, num_of_points, 5)
        # Nomralize the target_cls
        target_cls = torch.sum(target_cls, dim=1) # shape = (b, num_of_points, 5)
        # Set the background class to 1
        target_cls[..., 0] = torch.clamp(1 - torch.sum(target_cls[..., 1:], dim=-1), 0, 1)
        target_cls = target_cls / torch.sum(target_cls, dim=-1, keepdim=True) # shape = (b, num_of_points, 5)
        ...
        return target_offset, target_shape, target_bboxes, target_scores.unsqueeze(-1), mask_ignore.unsqueeze(-1), target_cls
