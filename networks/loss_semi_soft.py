...
class Unsupervised_DetectionLoss(nn.Module):
    ...
    def cls_loss(pred: torch.Tensor, target, mask_ignore, background_mask, soft_prob, alpha = 0.75 , gamma = 2.0, num_neg = 10000, 
        ...
        for j in range(batch_size):
            ...
            neg_mask = torch.logical_and(record_targets == 0, background_mask[j])
            soft_mask = torch.logical_and(record_targets == 0, ~background_mask[j])
            if num_positive_pixels > 0:
            ...
            if torch.sum(soft_mask) > 0:
                ...
                a1 = 0.1
                a2 = 0.9
                beta = ((soft_prob_b - pred_prob).abs() ** 1.5)
