...
class DetectionLoss(nn.Module):
    ...
    def cls_loss(pred: torch.Tensor, target, mask_ignore, alpha = 0.75 , gamma = 2.0, num_neg = 10000, num_hard = 100, neg_pos_ratio = 100, fn_weight = 4.0, fn_threshold = 0.8, 
        ...
        for j in range(batch_size):
            ...
            target_soft_b = target_b.clone()
            target_b = torch.where(torch.ge(target_b, 1.), 1.0, 0.0)
            ...
            cls_loss = torch.where(torch.ge(target_soft_b, 1), cls_loss * target_soft_b, cls_loss)
    ...
    def target_proprocess(annotations: torch.Tensor, 
        ...
        shape[2] = 11
        annotations_new = -1 * torch.ones(shape, device=device)
        ...
        for sample_i in range(batch_size):
            ...
            for s in range(len(gt_bboxes)):
                ...
                if (percent > 0.1) and (nw*nh*nd >= 15):
                    ...
                    size = (nd * nh * nw)
                    bbox = torch.from_numpy(np.array([float(z1 + 0.5 * nd), float(y1 + 0.5 * nh), float(x1 + 0.5 * nw), float(nd), float(nh), float(nw), float(spacing_z), float(spacing_y), float(spacing_x), float(size), 0])).to(device)
    ...
    def get_pos_target(annotations: torch.Tensor,
        ...
        nodule_size = annotations[:, :, -2].clone()
        low_size = 250
        small_nodule_weight = torch.where(nodule_size >= low_size, torch.tensor(1.0, device=nodule_size.device), 1.0 + 3.0 * (low_size - nodule_size) / low_size)
        
        high_size1 = 1000
        high_size2 = 2000
        large_nodule_weight = torch.where(nodule_size <= high_size1, torch.tensor(1.0, device=nodule_size.device), 1.0 + 3.0 * torch.clamp((nodule_size - high_size1) / (high_size2 - high_size1), min=0.0, max=1.0))
        
        nodule_weight = torch.maximum(small_nodule_weight, large_nodule_weight)
        nodule_weight = torch.repeat_interleave(nodule_weight.unsqueeze(-1), repeats=pos_target_topk, dim=-1)
        
        mask_topk = torch.zeros_like(distance, device=distance.device).scatter_(-1, topk_inds[:, :, :pos_target_topk], nodule_weight) # (b, num_annotation, num_of_points)
