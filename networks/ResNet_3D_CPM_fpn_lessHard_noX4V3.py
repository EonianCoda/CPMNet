...
class ClsRegHead(nn.Module):
    ...
    def forward(self, x):
        shape_feat = self.conv_r(x)
        offset_feat = self.conv_o(x)
        cls_feat = self.conv_s(x)
        
        return cls_feat, shape_feat, offset_feat
...
class Resnet18(nn.Module):
    def __init__(self, n_channels=1, n_blocks=[2, 3, 3, 3], n_filters=[64, 96, 128, 160], stem_filters=32,
        ...
        if out_stride == 4:
            ...
            # Head
            self.head1 = ClsRegHead(in_channels=feature_size, feature_size=feature_size, conv_num=3, norm_type=head_norm, act_type=act_type)
        elif out_stride == 2:
            feature_size = n_filters[0]
            self.fpn = FPN3D(n_filters, feature_size=feature_size, norm_type=norm_type, act_type=act_type)
            # Head
            self.head1 = ClsRegHead(in_channels=feature_size, feature_size=feature_size, conv_num=3, norm_type=head_norm, act_type=act_type)
            self.head2 = ClsRegHead(in_channels=feature_size, feature_size=feature_size, conv_num=3, norm_type=head_norm, act_type=act_type)
    ...
    def forward(self, inputs):
        ...
        "decode"
        cls_feat1, shape_feat1, offset_feat1 = self.head1(feats[0])
        cls_feat2, shape_feat2, offset_feat2 = self.head2(feats[1])
        
        cls_feat2 = self.upsample(cls_feat2)
        shape_feat2 = self.upsample(shape_feat2)
        offset_feat2 = self.upsample(offset_feat2)
        
        cls_feat1 = cls_feat1 + cls_feat2
        shape_feat1 = shape_feat1 + shape_feat2
        offset_feat1 = offset_feat1 + offset_feat2
        
        Shape = self.shape_output(shape_feat1)
        Offset = self.offset_output(offset_feat1)
        Cls = self.cls_output(cls_feat1)
        
        out = {}
        out['Cls'] = Cls
        out['Shape'] = F.relu(Shape)
        out['Offset'] = Offset
    ...
    def _init_weight(self):
        ...
        prior = 0.01
        nn.init.constant_(self.cls_output.weight, 0)
        nn.init.constant_(self.cls_output.bias, -math.log((1.0 - prior) / prior))

        nn.init.constant_(self.shape_output.weight, 0)
        nn.init.constant_(self.shape_output.bias, 0.5)

        nn.init.constant_(self.offset_output.weight, 0)
        nn.init.constant_(self.offset_output.bias, 0.05)
