...
class MSC(nn.Module):
    ...
    def __init__(self, input_channel, out_channel, norm_type='batchnorm', act_type='ReLU'):
        super(MSC, self).__init__()
        feat_channel = out_channel // 2
        self.conv_1 = ConvBlock(input_channel, feat_channel, kernel_size=1, stride=1, norm_type=norm_type, act_type='none')
        self.conv_3 = ConvBlock(input_channel, feat_channel, kernel_size=3, stride=1, norm_type=norm_type, act_type='none')
        self.conv_5 = ConvBlock(input_channel, feat_channel, kernel_size=5, stride=1, norm_type=norm_type, act_type='none')

        self.transition = ConvBlock(feat_channel * 3, out_channel, kernel_size=1, norm_type=norm_type, act_type=act_type)
    def forward(self, inputs):
        feat_1 = self.conv_1(inputs)
        feat_3 = self.conv_3(inputs)
        feat_5 = self.conv_5(inputs)
        
        out = torch.cat([feat_1, feat_3, feat_5], dim=1)
        out = self.transition(out)
        return out
...
class ClsRegHead(nn.Module):
    def __init__(self, in_channels, feature_size=96, conv_num=2,
        ...
        for i in range(conv_num):
            if i == 0:
                conv_s.append(MSC(in_channels, feature_size, norm_type=norm_type, act_type=act_type))
                conv_r.append(MSC(in_channels, feature_size, norm_type=norm_type, act_type=act_type))
                conv_o.append(MSC(in_channels, feature_size, norm_type=norm_type, act_type=act_type))
            else:
                conv_s.append(MSC(feature_size, feature_size, norm_type=norm_type, act_type=act_type))
                conv_r.append(MSC(feature_size, feature_size, norm_type=norm_type, act_type=act_type))
                conv_o.append(MSC(feature_size, feature_size, norm_type=norm_type, act_type=act_type))
