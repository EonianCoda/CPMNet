...
class ELAN_Layer(nn.Module):
    def __init__(self, n_stages: int, in_channels: int, out_channels: int, stride=1, norm_type='batchnorm', act_type='ReLU', se=False, ratio=4):
...
class Resnet18(nn.Module):
    def __init__(self, n_channels=1, n_blocks=[2, 3, 3, 3], n_filters=[64, 96, 128, 160], stem_filters=32,
        ...
        self.block33_res = ELAN_Layer(1, n_filters[2], n_filters[2], norm_type=norm_type, act_type=act_type, se=se)
        ...
        self.block22_res = ELAN_Layer(1, n_filters[1], n_filters[1], norm_type=norm_type, act_type=act_type, se=se)
