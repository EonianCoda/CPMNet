...
class SubNetFC(nn.Module):
    def __init__(self, m_top_k = 4, inner_channel = 64, add_mean = True):
        ...
        total_dim = (m_top_k + 1) * 3 if add_mean else m_top_k * 3
        self.reg_conf = nn.Sequential(nn.Linear(total_dim, inner_channel),
    ...
    def forward(self, x):
        bs, num_points, _ , _ = x.shape
        prob_topk, _ = x.topk(self.m_top_k, dim = -1)  # shape=[bs, d*h*w, 3, m_top_k]
        if self.add_mean:
            stat = torch.cat([prob_topk, prob_topk.mean(dim = -1, keepdim = True)], dim=-1) # shape=[bs, d*h*w, 3, m_top_k+1]
        else:
            stat = prob_topk # shape=[bs, d*h*w, 3, m_top_k]
        quality_score = self.reg_conf(stat.view(bs, num_points, -1)) # shape=[bs, d*h*w, 1]
        quality_score = quality_score.squeeze(-1) # shape=[bs, d*h*w]
...
class ClsRegHead(nn.Module):
    ...
    def forward(self, x):
        Shape_feat = self.scale(self.shape_output(self.conv_r(x))) # [bs, 3*(reg_max+1), z, y, x]
        bs, n, d, h, w = Shape_feat.shape
        num_points = d * h * w
        Shape_feat = Shape_feat.view(bs, n, num_points).permute(0, 2, 1).contiguous().view(bs, num_points, 3, -1) # [bs, z*y*x, 3, reg_max+1]
        Shape_feat = F.softmax(Shape_feat, dim = -1) # [bs, z*y*x, 3, reg_max+1]
        
        Offset = self.offset_output(self.conv_o(x))
        Cls = self.cls_output(self.conv_s(x)) # shape = [bs, 1, z, y, x]
        dict1 = {}
        
        cls_weight = self.subnet(Shape_feat).sigmoid() # [bs, d*h*w]
        cls_weight = cls_weight.view(bs, 1, d, h, w)
        dict1['Cls'] = Cls * cls_weight
