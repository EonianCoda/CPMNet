...
class Resnet18(nn.Module):
    def __init__(self, n_channels=1, n_blocks=[2, 3, 3, 3], n_filters=[64, 96, 128, 160], stem_filters=32,
        ...
        if out_stride == 4:
            ...
            feat_size = 96
            self.fpn = FPN3D(n_filters, feature_size=feat_size, norm_type=norm_type, act_type=act_type)
            # Head
            self.head = ClsRegHead(in_channels=feat_size, feature_size=feat_size, conv_num=3, norm_type=head_norm, act_type=act_type)
