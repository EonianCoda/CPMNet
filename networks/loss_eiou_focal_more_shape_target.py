...
class DetectionLoss(nn.Module):
    ...
    def bbox_iou(box1, box2, DIoU=True, eps = 1e-7):
        ...
        eiou = diou - loss_asp
        eiou_loss = 1 - eiou
        focal_eiou = (iou.detach() ** 0.5) * eiou_loss
        return focal_eiou
    ...
    def get_pos_target(annotations: torch.Tensor,
        ...
        shape_topk = torch.zeros_like(distance, device=distance.device).scatter_(-1, topk_inds[:, :, :pos_target_topk * 2], 1) # (b, num_annotation, num_of_points)
        ...
        shape_pos = shape_topk * mask_gt.unsqueeze(-1)
        ...
        shape_target, _ = torch.max(shape_pos, 1)
        ...
        return target_offset, target_shape, target_bboxes, target_scores.unsqueeze(-1), mask_ignore.unsqueeze(-1), shape_target.unsqueeze(-1)
    ...
    def forward(self, 
        ...
        target_offset, target_shape, target_bboxes, target_scores, mask_ignore, shape_target = self.get_pos_target(annotations = process_annotations,
        ...
        fg_mask = shape_target.squeeze(-1).bool()
        if fg_mask.sum() == 0:
