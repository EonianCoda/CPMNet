...
class Resnet18(nn.Module):
    def __init__(self, n_channels=1, n_blocks=[2, 3, 3, 3], n_filters=[64, 96, 128, 160], stem_filters=32,
        ...
        self.P4_1 = ConvBlock(n_filters[2], n_filters[3], kernel_size=1, norm_type=norm_type, act_type='none')
        
        # feat_8x = F.upsample(feats[3], scale_factor=2, mode='nearest') + feats[2]
        ...
        # Decoder
        if out_stride == 4:
            feature_size = n_filters[1]
            self.fpn = FPN3D(n_filters[1:], feature_size=feature_size, norm_type=norm_type, act_type=act_type)
            self.P5_upsampled = UpsamplingDeconvBlock(feature_size, feature_size, stride=2, norm_type=norm_type, act_type='none')
            self.P4_1 = ConvBlock(feature_size, feature_size, kernel_size=3, norm_type=norm_type, act_type='none')
            self.P4_upsampled = UpsamplingDeconvBlock(feature_size, feature_size, stride=2, norm_type=norm_type, act_type='none')
            self.P3_1 = ConvBlock(feature_size, feature_size, kernel_size=1, norm_type=norm_type, act_type='none')
    ...
    def forward(self, inputs):
        ...
        feat_8x = self.P5_upsampled(feats[2]) + feats[1]
        feat_8x = self.P4_1(feat_8x)
        feat_4x = self.P4_upsampled(feat_8x)
        feat_4x = self.P3_1(feat_4x)
        
        out1 = self.head(feats[0])
        out2 = self.head(feat_4x)
        if self.training:
            cls_pos_loss, cls_neg_loss, shape_loss, offset_loss, iou_loss = self.detection_loss(out1, labels, device=self.device)
            cls_pos_loss2, cls_neg_loss2, shape_loss2, offset_loss2, iou_loss2 = self.detection_loss(out2, labels, device=self.device)
            
            ratio = 0.5
            cls_pos_loss = ratio * cls_pos_loss + (1 - ratio) * cls_pos_loss2
            cls_neg_loss = ratio * cls_neg_loss + (1 - ratio) * cls_neg_loss2
            shape_loss = ratio * shape_loss + (1 - ratio) * shape_loss2
            offset_loss = ratio * offset_loss + (1 - ratio) * offset_loss2
            iou_loss = ratio * iou_loss + (1 - ratio) * iou_loss2
...
class DetectionPostprocess(nn.Module):
    ...
    def forward(self, output, device, is_logits=True):
        ...
        out1['Cls'] = (out1['Cls'] + out2['Cls']) / 2
        out1['Shape'] = (out1['Shape'] + out2['Shape']) / 2
        out1['Offset'] = (out1['Offset'] + out2['Offset']) / 2
