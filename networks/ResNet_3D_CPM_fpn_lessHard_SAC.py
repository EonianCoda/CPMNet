...
class SAC(nn.Module):
    def __init__(self, input_channel, out_channel, norm_type='batchnorm'):
        super(SAC, self).__init__()
        self.conv_1 = ConvBlock(input_channel, out_channel, kernel_size=3, stride=1, norm_type=norm_type, act_type='none')
        self.conv_3 = ConvBlock(input_channel, out_channel, kernel_size=3, stride=1, dilation=2, norm_type=norm_type, act_type='none')
        self.conv_5 = ConvBlock(input_channel, out_channel, kernel_size=3, stride=1, dilation=3, norm_type=norm_type, act_type='none')
...
class FPN3D(nn.Module):
    def __init__(self, n_filters, feature_size=64, norm_type='batchnorm', act_type='ReLU'):
        ...
        if len(n_filters) == 4:
            ...
            self.P2_2 = SAC(feature_size, feature_size, norm_type=norm_type)
        else:
            self.P3_2 = SAC(feature_size, feature_size, norm_type=norm_type)
