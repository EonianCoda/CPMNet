...
class DetectionLoss(nn.Module):
    ...
    def cls_loss(pred: torch.Tensor, target, mask_ignore, alpha = 0.75 , gamma = 2.0, num_neg = 10000, num_hard = 100, 
        ...
        for j in range(batch_size):
            ...
            if num_positive_pixels > 0:
                ...
                if hard_fp_threshold != -1 and hard_fp_weight != -1:
                    hard_FP_thre1, hard_FP_thrs2 = 0.5, 0.8
                    hard_FP_w1, hard_FP_w2 = 1.5, 2.5
        ...
        if len(cls_pos_losses) == 0:
            ...
            cls_pos_loss = torch.mean(torch.stack(cls_pos_losses)) #/ batch_size
            
        if len(cls_neg_losses) == 0:
            cls_neg_loss = torch.tensor(0.0, device=pred.device)
        else:
            cls_neg_loss = torch.mean(torch.stack(cls_neg_losses)) #/ batch_size
...
class DetectionPostprocess(nn.Module):
    ...
    def forward(self, output, lobe, device, is_logits=True):
        Cls = output['Cls']
        Shape = output['Shape']
        Offset = output['Offset']
        batch_size = Cls.size()[0]
        ...
        Cls[lobe == 0] = -20 # ignore the lobe 0, -20 indicates the background, sigmoid(-20) is close to 0
