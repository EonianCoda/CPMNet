...
class ELAN_Layer(nn.Module):
    def __init__(self, n_stages: int, in_channels: int, out_channels: int, stride=1, norm_type='batchnorm', act_type='ReLU', se=False):
        ...
        split_feat = in_channels // 4
        
        self.m1_stem1 = ConvBlock(split_feat, feat_size, kernel_size=1, norm_type=norm_type, act_type='none')
        self.m1_stem2 = ConvBlock(split_feat, feat_size, kernel_size=1, norm_type=norm_type, act_type='none')
        for i in range(n_stages):
            setattr(self, f'm1_conv{i}', BasicBlockNew(feat_size, feat_size, norm_type=norm_type, act_type=act_type, se=se))
        
        self.m2_stem1 = ConvBlock(split_feat, feat_size, kernel_size=1, norm_type=norm_type, act_type='none')
        self.m2_stem2 = ConvBlock(split_feat, feat_size, kernel_size=1, norm_type=norm_type, act_type='none')
        for i in range(n_stages):
            setattr(self, f'm2_conv{i}', BasicBlockNew(feat_size, feat_size, norm_type=norm_type, act_type=act_type, se=se))
        
        self.m1_trans = ConvBlock(feat_size * (n_stages + 2), out_channels, kernel_size=1, act_type=act_type, norm_type='none')
        self.m2_trans = ConvBlock(feat_size * (n_stages + 2), out_channels, kernel_size=1, act_type=act_type, norm_type='none')
        
        self.act = act_layer(act_type)
        
    def forward(self, x):
        x1, x2, x3, x4 = torch.chunk(x, 4, 1)
        y1 = [self.m1_stem1(x1), self.m1_stem2(x2)]
        y2 = [self.m2_stem1(x3), self.m2_stem2(x4)]
        for i in range(self.n_stages):
            y1.append(getattr(self, f'm1_conv{i}')(y1[-1]))
            y2.append(getattr(self, f'm2_conv{i}')(y2[-1]))
        y1 = self.m1_trans(torch.cat(y1, 1))
        y2 = self.m2_trans(torch.cat(y2, 1))
...
class StemBlock(nn.Module):
    def __init__(self, in_channels, out_channels, norm_type='none', act_type='ReLU'):
        ...
        self.conv1 = ConvBlock(in_channels, feat_size, kernel_size=1, norm_type=norm_type, act_type='none')
        self.conv2 = ConvBlock(in_channels, feat_size, kernel_size=3, norm_type=norm_type, act_type='none')
        self.conv3 = ConvBlock(in_channels, feat_size, kernel_size=3, dilation=2, norm_type=norm_type, act_type='none')
        self.conv4 = ConvBlock(in_channels, feat_size, kernel_size=3, dilation=3, norm_type=norm_type, act_type='none')
        self.trans = ConvBlock(feat_size * 4, out_channels, kernel_size=1, norm_type=norm_type, act_type=act_type)
...
class Resnet18(nn.Module):
    def __init__(self, n_channels=1, n_blocks=[2, 3, 3, 3], n_filters=[64, 96, 128, 160], stem_filters=32,
        ...
        self.in_conv = stem_block(n_channels, stem_filters, norm_type=norm_type, act_type=act_type)
