...
class StemBlock(nn.Module):
    def __init__(self, in_channels, out_channels, norm_type='none', act_type='ReLU', num_conv=3):
        ...
        self.stem1 = ConvBlock(in_channels, feat_size, kernel_size=1, norm_type=norm_type, act_type='none')
        self.stem2 = ConvBlock(in_channels, feat_size, kernel_size=1, norm_type=norm_type, act_type='none')
        
        for i in range(num_conv):
            setattr(self, f'conv{i}', ConvBlock(feat_size, feat_size, kernel_size=3, norm_type=norm_type, act_type='none'))
        
        self.trans = ConvBlock(feat_size * (num_conv + 2), out_channels, kernel_size=1, norm_type=norm_type, act_type=act_type)
    
    def forward(self, x):
        y = [self.stem1(x), self.stem2(x)]
...
class DownsamplingConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=2, norm_type='batchnorm', act_type='ReLU'):
        ...
        self.dw = nn.MaxPool3d(kernel_size=stride, stride=stride)
        self.dw_conv = ConvBlock(in_channels, out_channels // 2, 1, norm_type=norm_type, act_type=act_type)
        
        self.conv_1 = ConvBlock(in_channels, out_channels // 2, 1, norm_type=norm_type, act_type=act_type)
        self.conv_2 = ConvBlock(out_channels // 2, out_channels // 2, 3, norm_type=norm_type, act_type=act_type, stride=stride)

    def forward(self, x):
        x1 = self.dw_conv(self.dw(x))
        x2 = self.conv_2(self.conv_1(x))
        x = torch.cat([x1, x2], 1)
        return x
