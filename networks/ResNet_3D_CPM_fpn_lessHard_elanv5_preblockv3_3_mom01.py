...
from .modules import SELayer, Identity, act_layer

def norm_layer3d(norm_type, num_features):
    if norm_type == 'batchnorm':
        return nn.BatchNorm3d(num_features=num_features, momentum=0.1)
...
class StemBlock(nn.Module):
    def __init__(self, in_channels, out_channels, norm_type='none', act_type='ReLU', num_conv=3):
        ...
        self.stem1 = ConvBlock(in_channels, feat_size, kernel_size=1, norm_type=norm_type, act_type=act_type)
        self.stem2 = ConvBlock(in_channels, feat_size, kernel_size=1, norm_type=norm_type, act_type=act_type)
        
        for i in range(num_conv):
            setattr(self, f'conv{i}', ConvBlock(feat_size, feat_size, kernel_size=3, norm_type=norm_type, act_type=act_type))
