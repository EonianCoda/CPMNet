...
class Unsupervised_DetectionLoss(nn.Module):
    ...
    def forward(self, 
        ...
        gt_scores = Cls_label.view(batch_size, 1, -1) # (b, 1, num_points)
        gt_shapes = Shape_label.view(batch_size, 3, -1) # (b, 3, num_points)
        gt_offsets = Offset_label.view(batch_size, 3, -1)
        ...
        gt_scores = gt_scores.permute(0, 2, 1).contiguous()
        gt_shapes = gt_shapes.permute(0, 2, 1).contiguous()
        gt_offsets = gt_offsets.permute(0, 2, 1).contiguous()
        
        # generate center points. Only support single scale feature
        anchor_points, stride_tensor = make_anchors(Cls, self.crop_size, 0) # shape = (num_anchors, 3)
        # predict bboxes (zyxdhw)
        pred_bboxes = bbox_decode(anchor_points, pred_offsets, pred_shapes, stride_tensor) # shape = (b, num_anchors, 6)
        target_bboxes = bbox_decode(anchor_points, gt_offsets, gt_shapes, stride_tensor) # shape = (b, num_anchors, 6)
...
class DetectionPostprocess(nn.Module):
    ...
    def forward(self, output, device, threshold=None, is_logits=True):
        Cls = output['Cls']
        Shape = output['Shape']
        Offset = output['Offset']
        batch_size = Cls.size()[0]
