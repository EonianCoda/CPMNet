...
from utils.box_utils import nms_3D_matched, bbox_decode, make_anchors
    ...
    def forward(self, output, device, is_logits=True, lobe_mask=None):
        ...
        for j in range(batch_size):
            ...
            if keep_box_n > 0:
                ...
                keep, num_matched = nms_3D_matched(det[:, 1:], overlap=self.nms_threshold, top_k=self.nms_topk)
                dets[j][:len(keep)] = det[keep.long()]
                all_num_matched.append(num_matched)
            else:
