...
class ClsRegHead(nn.Module):
    ...
    def forward(self, cls_feat, reg_feat):
        Shape = self.shape_output(self.conv_r(reg_feat))
        Offset = self.offset_output(self.conv_o(reg_feat))
        Cls = self.cls_output(self.conv_s(cls_feat))
        dict1 = {}
        dict1['Cls'] = Cls
        dict1['Shape'] = Shape
        dict1['Offset'] = Offset
...
class Resnet18(nn.Module):
    def __init__(self, n_channels=1, n_blocks=[2, 3, 3, 3], n_filters=[64, 96, 128, 160], stem_filters=32,
        ...
        if out_stride == 4:
            ...
            # Head
            self.reg_upsample1 = UpsamplingDeconvBlock(feature_size, feature_size, stride=2, norm_type=norm_type, act_type=act_type)
            self.reg_conv1 = ConvBlock(feature_size, feature_size, kernel_size=3, norm_type=norm_type, act_type='none')
            self.reg_upsample2 = UpsamplingDeconvBlock(feature_size, feature_size, stride=2, norm_type=norm_type, act_type=act_type)
            self.reg_conv2 = ConvBlock(feature_size, feature_size, kernel_size=3, norm_type=norm_type, act_type='none')
    ...
    def forward(self, inputs):
        ...
        # P3_x, P4_x, P5_x = feats
        cls_feat = feats[0]
        reg_feat = self.reg_upsample1(feats[2]) + feats[1]
        reg_feat = self.reg_conv1(reg_feat)
        reg_feat = self.reg_upsample2(reg_feat) + feats[0]
        reg_feat = self.reg_conv2(reg_feat)
        "decode"
        out = self.head(cls_feat, reg_feat)
        if self.training:
