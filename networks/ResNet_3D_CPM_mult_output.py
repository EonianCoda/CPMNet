...
class Resnet18(nn.Module):
    def __init__(self, n_channels=1, n_blocks=[2, 3, 3, 3], n_filters=[64, 96, 128, 160], stem_filters=32,
        ...
        self.block33_2 = LayerBasic(1, n_filters[2], n_filters[2], norm_type=norm_type, act_type=act_type, se=se)

        self.block22_up = up_block(n_filters[2], n_filters[1], norm_type=norm_type, act_type=act_type)
        self.block22_up2 = up_block(n_filters[2], n_filters[1], norm_type=norm_type, act_type=act_type)
        ...
        if out_stride == 4:
            ...
            self.block22_2 = LayerBasic(1, n_filters[1], n_filters[1], norm_type=norm_type, act_type=act_type, se=se)
            
            self.block11_up = up_block(n_filters[1], n_filters[0], norm_type=norm_type, act_type=act_type)
            self.block11_up_2 = up_block(n_filters[1] * 2, n_filters[0], norm_type=norm_type, act_type=act_type)
    ...
    def forward(self, inputs):
        ...
        enc1 = self.block1(x)
        x = self.block1_dw(enc1)

        enc2 = self.block2(x)
        x = self.block2_dw(enc2)

        if self.dropout is not None:
            enc2 = self.dropout(enc2)

        enc3 = self.block3(x)
        x = self.block3_dw(enc3)
        ...
        "decode"
        x = self.block33_up(x)
        enc3 = self.block33_res(enc3)
        x = torch.cat([x, enc3], dim=1)
        x = self.block33(x)
        x2 = self.block33_2(x)
        
        x = self.block22_up(x)
        enc2 = self.block22_res(enc2)
        x = torch.cat([x, enc2], dim=1)
        x = self.block22(x)
        x2 = self.block22_up2(x2)
        
        if hasattr(self, 'block11_up'):
            x2 = torch.cat([x2, self.block22_2(x)], dim=1)
            x = self.block11_up(x)
            enc1 = self.block11_res(enc1)
            x = torch.cat([x, enc1], dim=1)
            x = self.block11(x)
            x2 = self.block11_up_2(x2)
        ...
        if self.training:
            ...
            pos_cls_loss2, neg_cls_loss2, shape_loss2, offset_loss2, iou_loss2 = self.detection_loss(out2, labels, device=self.device)
            
            pos_cls_loss = pos_cls_loss * ratio + pos_cls_loss2 * (1 - ratio)
            neg_cls_loss = neg_cls_loss * ratio + neg_cls_loss2 * (1 - ratio)
            shape_loss = shape_loss * ratio + shape_loss2 * (1 - ratio)
            offset_loss = offset_loss * ratio + offset_loss2 * (1 - ratio)
            iou_loss = iou_loss * ratio + iou_loss2 * (1 - ratio)
    ...
    def _init_weight(self):
        ...
        nn.init.constant_(self.head2.cls_output.weight, 0)
        nn.init.constant_(self.head2.cls_output.bias, -math.log((1.0 - prior) / prior))
        
        nn.init.constant_(self.head2.shape_output.weight, 0)
        nn.init.constant_(self.head2.shape_output.bias, 0.5)
        
        nn.init.constant_(self.head2.offset_output.weight, 0)
        nn.init.constant_(self.head2.offset_output.bias, 0.05)
...
class DetectionPostprocess(nn.Module):
    ...
    def get_scores(self, output, is_logits=True):
        Cls = output['Cls']
        Shape = output['Shape']
        Offset = output['Offset']
    ...
    def forward(self, output, device, is_logits=True):
        ...
        topk_scores, topk_indices, pred_bboxes = self.get_scores(out1, is_logits)
        topk_scores2, topk_indices2, pred_bboxes2 = self.get_scores(out2, is_logits)
