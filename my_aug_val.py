...
### data ###
from dataload.my_dataset_crop_fast import AugDetDataset
from dataload.collate import aug_infer_collate_fn
from dataload.split_combine import AugSplitComb
from torch.utils.data import DataLoader
import transform as transform

from logic.val import aug_val
...
def val_data_prepare(args):
    ...
    if args.data_norm_method == 'none':
        ...
        pad_value = -1
    split_comber = AugSplitComb(crop_size=crop_size, overlap_size=overlap_size, pad_value=pad_value)
    test_dataset = AugDetDataset(series_list_path = args.val_set, SplitComb=split_comber, image_spacing=IMAGE_SPACING, norm_method=args.data_norm_method)
    val_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.batch_size, collate_fn=aug_infer_collate_fn, pin_memory=True)
