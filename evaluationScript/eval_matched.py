...
class Evaluation:
    ...
    def evaluation(self, preds: List[List[Any]], save_dir: str, froc_det_thresholds = List[float]):
        ...
        for pred in preds:
            series_name, ctr_x, ctr_y, ctr_z, prob, w, h, d, num_match, avg_num_match = pred
            all_pred_cands[series_name].append(self._build_nodule_finding(series_name, ctr_x, ctr_y, ctr_z, w, h, d, prob = prob, num_match = num_match, avg_num_match = avg_num_match))
                ...
                if len(all_ious) != 0:
                    ...
                    for gt_idx, gt_nodule in enumerate(gt_nodules):
                        ...
                        if np.any(match_mask): # TP
                            ...
                            setattr(gt_nodule, 'num_match', getattr(match_cand, 'num_match', 0))
                            setattr(gt_nodule, 'avg_num_match', getattr(match_cand, 'avg_num_match', 0))
    ...
    def _write_predicitions(self, save_dir: str, froc: FROC):
        ...
        header = "series_name,ctr_x,ctr_y,ctr_z,w,h,d,prob, nodule_type, match_iou, is_gt, num_matched, avg_num_matched, gt_ctr_x, gt_ctr_y, gt_ctr_z, gt_w, gt_h, gt_d\n"
        lines = [header]
        ...
        for series_name in sorted(series_name_cands.keys()):
            for cand in sorted(series_name_cands[series_name], key=cmp_to_key(sort_by_x)):
                ...
                prob = cand.prob
                num_match = getattr(cand, 'num_match', 0)
                avg_num_match = getattr(cand, 'avg_num_match', 0)
                nodule_type = cand.nodule_type
                match_iou = cand.match_iou
                is_gt = cand.is_gt
                if not is_gt or (is_gt and prob == -1):
                    pred_line = "{},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{},{:.3f},{},{},{:.3f}\n".format(series_name, ctr_x, ctr_y, ctr_z, w, h, d, prob, nodule_type, match_iou, is_gt, num_match, avg_num_match)
                else:
                    ...
                    pred_line = "{},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{},{:.3f},{},{},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f}\n".format(series_name, ctr_x, ctr_y, ctr_z, w, h, d, prob, nodule_type, match_iou, is_gt, num_match, avg_num_match, gt_ctr_x, gt_ctr_y, gt_ctr_z, gt_w, gt_h, gt_d)
