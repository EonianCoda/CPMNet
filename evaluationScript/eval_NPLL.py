...
class Evaluation:
    ...
    def evaluation(self, preds: List[List[Any]], save_dir: str, froc_det_thresholds = List[float]):
        ...
        for pred in preds:
            series_name, ctr_x, ctr_y, ctr_z, prob, w, h, d, w_std, h_std, d_std = pred
            all_pred_cands[series_name].append(self._build_nodule_finding(series_name, ctr_x, ctr_y, ctr_z, w, h, d, prob = prob, w_std = w_std, h_std = h_std, d_std = d_std))
    ...
    def _write_predicitions(self, save_dir: str, froc: FROC):
        ...
        header = "series_name,ctr_x,ctr_y,ctr_z,w,h,d, w_std, h_std, d_std, prob, nodule_type, match_iou, is_gt, gt_ctr_x, gt_ctr_y, gt_ctr_z, gt_w, gt_h, gt_d\n"
        lines = [header]
        ...
        for series_name in sorted(series_name_cands.keys()):
            for cand in sorted(series_name_cands[series_name], key=cmp_to_key(sort_by_x)):
                ...
                if not is_gt or (is_gt and prob == -1):
                    if is_gt:
                        w_std, h_std, d_std = 0.0, 0.0, 0.0
                    else:
                        w_std, h_std, d_std = cand.w_std, cand.h_std, cand.d_std
                    pred_line = "{},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{},{:.3f},{}\n".format(series_name, ctr_x, ctr_y, ctr_z, w, h, d, w_std, h_std, d_std, prob, nodule_type, match_iou, is_gt)
                else:
                    ...
                    w_std, h_std, d_std = pred_cand.w_std, pred_cand.h_std, pred_cand.d_std
                    pred_line = "{},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{},{:.3f},{},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f}\n".format(series_name, ctr_x, ctr_y, ctr_z, w, h, d, w_std, h_std, d_std, prob, nodule_type, match_iou, is_gt, gt_ctr_x, gt_ctr_y, gt_ctr_z, gt_w, gt_h, gt_d)
