...
from networks.ResNet_3D_CPM_classification import Resnet18, DetectionPostprocess, DetectionLoss
### data ###
from dataload.dataset_classification import TrainDataset, DetDataset
from dataload.utils import get_image_padding_value
from dataload.collate import train_classification_collate_fn, infer_collate_fn
...
### logic ###
from logic.train_classification import train
...
def get_args():
    ...
    parser.add_argument('--lambda_cls', type=float, default=1.0, help='weights of cls loss')
    parser.add_argument('--lambda_obj', type=float, default=4.0, help='weights of obj loss')
...
def build_train_augmentation(args, crop_size: Tuple[int, int, int], pad_value: float, blank_side: int):
    ...
    transform_list_train.append(transform.ClassificationCoordToAnnot())
...
def get_train_dataloder(args, blank_side=0) -> DataLoader:
    ...
    train_loader = DataLoader(train_dataset, 
                              ...
                              shuffle=True,
                              collate_fn=train_classification_collate_fn,
