...
### data ###
from dataload.dataset_semi_tta_tracking_enhance_small import TrainDataset, DetDataset, UnLabeledDataset
# Build crop function
...
### logic ###
from logic.semi_threshold_tta_train_soft_tracking_enhance_small import train, burn_in_train
...
def get_args():
    ...
    # Semi hyper-parameters
    parser.add_argument('--small_size_threshold', type=int, default=200, help='small size threshold')
    ...
    parser.add_argument('--pseudo_tp_ratio', type=float, default=0.75, help='positive ratio in instance crop')
...
def get_train_dataloder(args, blank_side=0) -> DataLoader:
    ...
    logger.info('Small size threshold: {}'.format(args.small_size_threshold))
    train_dataset_u = UnLabeledDataset(series_list_path = args.unlabeled_train_set, crop_fn = crop_fn_train_u, image_spacing=IMAGE_SPACING, use_gt_crop=args.use_gt_crop,
                                       strong_aug=strong_aug, min_d=args.min_d, min_size = args.min_size, norm_method=args.data_norm_method, mmap_mode=mmap_mode, pseudo_remove_threshold = args.pseudo_remove_threshold, 
                                       pseudo_update_ema_alpha=args.pseudo_update_ema_alpha, use_rotate90=args.semi_use_rotate90, small_size_threshold=args.small_size_threshold)
...
def updata_pseudo_label(args, model, det_dataloader, device, detection_postprocess, updated_dataset, 
    ...
    for series_name, label in pseu_labels.items():
        prob = label[ALL_PROB]
        if len(prob) != 0:
            valid_mask = (prob > get_prob_threshold(label[ALL_RAD], prob_threshold))
            if len(valid_mask) == 0:
