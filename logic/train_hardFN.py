...
def train(args,
    ...
    for iter_i, sample in enumerate(dataloader):
        ...
        if mixed_precision:
            ...
            nodule_indices = sample['nodule_indices']
            
            updated_series_names = []
            updated_nodule_indices = []
            updated_nodule_probs = []
            for b_i, (series_name, nodule_index) in enumerate(zip(series_names, nodule_indices)):
                for i in range(len(nodule_index)):
                    z, y, x = np.clip((annot_np[b_i][i, :3] / 4), 0, 23).astype(np.int32)
                    prob = cls_output[b_i, 0][z, y, x]
                    updated_series_names.append(series_name)
                    updated_nodule_indices.append(nodule_index[i])
                    updated_nodule_probs.append(prob)
            del cls_output
