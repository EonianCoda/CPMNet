            ...
            for i in range(int(math.ceil(data.size(0) / batch_size))):
                ...
                if args.val_mixed_precision:
                    with torch.cuda.amp.autocast():
                        with torch.no_grad():
                            out_s2, out_s4 = model(input)
                else:
                    with torch.no_grad():
                        out_s2, out_s4 = model(input)
                # Ensemble the augmentations
                Cls_output = out_s2['Cls'] # (bs * num_aug, 1, 24, 24, 24)
                Shape_output = out_s2['Shape'] # (bs * num_aug, 3, 24, 24, 24)
                Offset_output = out_s2['Offset'] # (bs * num_aug, 3, 24, 24, 24)
                ...
                if args.apply_lobe:
                    ...
                    lobe = None
                out_s2 = {'Cls': Cls_output, 'Shape': Shape_output, 'Offset': Offset_output}
                
                # Ensemble the augmentations
                Cls_output = out_s4['Cls'] # (bs * num_aug, 1, 24, 24, 24)
                Shape_output = out_s4['Shape'] # (bs * num_aug, 3, 24, 24, 24)
                Offset_output = out_s4['Offset'] # (bs * num_aug, 3, 24, 24, 24)
                ...
                if args.apply_lobe:
                    ...
                    lobe = None
                out_s4 = {'Cls': Cls_output, 'Shape': Shape_output, 'Offset': Offset_output}
                
                output = detection_postprocess([out_s2, out_s4], device=device, is_logits=False, lobe_mask = lobe) #1, prob, ctr_z, ctr_y, ctr_x, d, h, w
