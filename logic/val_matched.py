...
from utils.box_utils import nms_3D, nms_3D_matched, nms_3D_matched_sum
from evaluationScript.eval_matched import Evaluation
...
def convert_to_standard_output(output: np.ndarray, num_matched: List[int], avg_num_matched: List[float], series_name: str) -> List[List[Any]]:
    """
    ...
    for j in range(output.shape[0]):
        preds.append([series_name, output[j, 4], output[j, 3], output[j, 2], output[j, 1], output[j, 7], output[j, 6], output[j, 5], num_matched[j], avg_num_matched[j]])
    return preds
    ...
    with get_progress_bar('Validation', len(val_loader)) as progress_bar:
        for sample in val_loader:
                ...
                with torch.no_grad():
                    if args.val_mixed_precision:
                        with torch.cuda.amp.autocast():
                            output = model(input)
                            output, num_matched = detection_postprocess(output, device=device, lobe_mask=lobe)
                    else:
                        output = model(input)
                        output, num_matched = detection_postprocess(output, device=device, lobe_mask=lobe) #1, prob, ctr_z, ctr_y, ctr_x, d, h, w
                outputlist.append(output.data.cpu().numpy())
                num_matched_list.extend(num_matched)
            ...
            for i in range(len(num_splits)):
                ...
                num_matched_cross_patch = np.concatenate(num_matched_list[start_idx:start_idx + n_split])
                # NMS
                if len(output) > 0:
                    keep, num_matched, avg_num_matched = nms_3D_matched_sum(output[:, 1:], num_matched_cross_patch, overlap=0.05, top_k=nms_keep_top_k)
                    output = output[keep]
                output = output.numpy()
            
                preds = convert_to_standard_output(output, num_matched, avg_num_matched, series_names[i])  
