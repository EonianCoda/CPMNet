...
def train_one_step_wrapper(memory_format):
    def train_one_step(args, model: nn.modules, sample: Dict[str, torch.Tensor], device: torch.device) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Compute loss
        cls_pos_loss, cls_neg_loss, shape_loss, shape_std_loss, offset_loss, iou_loss = model([image, labels])
        cls_pos_loss, cls_neg_loss, shape_loss, shape_std_loss, offset_loss, iou_loss = cls_pos_loss.mean(), cls_neg_loss.mean(), shape_loss.mean(), shape_std_loss.mean(), offset_loss.mean(), iou_loss.mean()
        loss = args.lambda_cls * (cls_pos_loss + cls_neg_loss) + args.lambda_shape * shape_loss + args.lambda_offset * offset_loss + args.lambda_iou * iou_loss + args.lambda_shape_std * shape_std_loss
        del image, labels
        return loss, cls_pos_loss, cls_neg_loss, shape_loss, shape_std_loss, offset_loss, iou_loss
...
def train(args,
    ...
    avg_shape_std_loss = AverageMeter()
    ...
    for iter_i, sample in enumerate(dataloader):
        if mixed_precision:
            with torch.cuda.amp.autocast():
                loss, cls_pos_loss, cls_neg_loss, shape_loss, shape_std_loss, offset_loss, iou_loss = train_one_step(args, model, sample, device)
        ...
        avg_shape_std_loss.update(shape_std_loss.item())
        ...
        if (iter_i + 1) % iters_to_accumulate == 0 or iter_i == total_num_steps - 1:
            ...
            progress_bar.set_postfix(loss = avg_loss.avg,
                                    ...
                                    shape_std_loss = avg_shape_std_loss.avg,
