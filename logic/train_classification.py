...
def train_one_step_wrapper(memory_format):
    def train_one_step(args, model: nn.modules, sample: Dict[str, torch.Tensor], device: torch.device) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Compute loss
        obj_loss, cls_loss, shape_loss, offset_loss, iou_loss = model([image, labels])
        obj_loss, cls_loss, shape_loss, offset_loss, iou_loss = obj_loss.mean(), cls_loss.mean(), shape_loss.mean(), offset_loss.mean(), iou_loss.mean()
        loss = args.lambda_obj * obj_loss + args.lambda_cls * cls_loss + args.lambda_shape * shape_loss + args.lambda_offset * offset_loss + args.lambda_iou * iou_loss
        return loss, cls_loss, obj_loss, shape_loss, offset_loss, iou_loss
    ...
    for iter_i, sample in enumerate(dataloader):
        if mixed_precision:
            with torch.cuda.amp.autocast():
                loss, cls_loss, obj_loss, shape_loss, offset_loss, iou_loss = train_one_step(args, model, sample, device)
        ...
        avg_obj_loss.update(obj_loss.item())
