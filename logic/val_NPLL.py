...
from evaluationScript.eval_NPLL import Evaluation
...
def convert_to_standard_output(output: np.ndarray, series_name: str) -> List[List[Any]]:
    ...
    ['seriesuid', 'coordX', 'coordY', 'coordZ', 'probability', 'w', 'h', 'd', 'w_std', 'h_std', 'd_std']
    """
    preds = []
    for j in range(output.shape[0]):
        preds.append([series_name, output[j, 4], output[j, 3], output[j, 2], output[j, 1], output[j, 7], output[j, 6], output[j, 5], output[j, 10], output[j, 9], output[j, 8]])
    return preds
    ...
    with get_progress_bar('Validation', len(val_loader)) as progress_bar:
        for sample in val_loader:
            ...
            for i in range(int(math.ceil(data.size(0) / batch_size))):
                ...
                with torch.no_grad():
                    if args.val_mixed_precision:
                        ...
                        output = model(input)
                        output = detection_postprocess(output, device=device, lobe_mask=lobe) #1, prob, ctr_z, ctr_y, ctr_x, d, h, w, d_std, h_std, w_std
            ...
            for i in range(len(num_splits)):
                ...
                output = torch.from_numpy(output).view(-1, 11)
                # Remove the padding
                ...
                # NMS
                if len(output) > 0:
                    keep = nms_3D(output[:, 1:8], overlap=0.05, top_k=nms_keep_top_k)
                    output = output[keep]
                output = output.numpy()
