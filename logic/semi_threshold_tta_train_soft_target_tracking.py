...
def unsupervised_train_one_step_wrapper(memory_format, loss_fn):
    def train_one_step(args, model: nn.modules, sample: Dict[str, torch.Tensor], background_mask, soft_prob, device: torch.device) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        soft_labels = sample['annot_soft'].to(device, non_blocking=True) # z, y, x, d, h, w, type[-1, 0]
        # Compute loss
        outputs = model(image)
        cls_pos_loss, cls_neg_loss, cls_soft_loss, shape_loss, offset_loss, iou_loss = loss_fn(outputs, labels, soft_labels, background_mask, soft_prob, device = device)
...
def train(args,
    ...
    select_more_soft_fg_crop = getattr(args, 'select_more_soft_fg_crop', False)
    if ema_buffer:
    ...
    if select_more_soft_fg_crop:
        logger.info('Get more bg crop with more soft pseudo label')
    ...
    with get_progress_bar('Train', num_iters) as progress_bar:
        for iter_i, sample_u in enumerate(dataloader_u):
            ...
            for i in range(int(math.ceil(weak_images.size(0) / TTA_BATCH_SIZE))):
                ...
                outputs_t_b = detection_postprocess(outputs_t_b, device=device, is_logits=False, lobe_mask = lobe, threshold = args.pseudo_soft_threshold, nms_topk=args.pseudo_nms_topk) #1, prob, ctr_z, ctr_y, ctr_x, d, h, w
            ...
            weak_spacings = weak_u_sample['spacing'] # shape = (bs,)
            transformed_annots = []
            transformed_soft_annots = []
            for batch_i in range(bs):
                # According prob to generate pseudo label
                probs_b = history_probs[batch_i]
                valid_mask = (probs_b > args.pseudo_label_threshold)
                soft_valid_mask = (probs_b > args.pseudo_background_threshold) & (probs_b <= args.pseudo_label_threshold)
                ...
                if np.count_nonzero(soft_valid_mask) == 0:
                    transformed_soft_annots.append(np.zeros((0, 10), dtype='float32'))
                else:
                    ctrs_b = history_ctrs[batch_i][soft_valid_mask].copy()
                    rads_b = history_rads[batch_i][soft_valid_mask].copy()
                    probs_b = history_probs[batch_i][soft_valid_mask].copy()
                    spacing = weak_spacings[batch_i]
                    ...
                    sample_dict = {'ctr': ctrs_b, 
                                    ...
                                    'prob': probs_b,
                                    'spacing': spacing}
                    transformed_soft_annots.append(coord_to_annot(sample_dict)['annot'])
            ...
            # Pad the soft pseudo label
            soft_valid_mask = np.array([len(annot) > 0 for annot in transformed_soft_annots], dtype=np.int32)
            soft_valid_mask = (soft_valid_mask == 1)
            
            max_num_annots = max(annot.shape[0] for annot in transformed_soft_annots)
            if max_num_annots > 0:
                transformed_annots_padded_soft = np.ones((len(transformed_soft_annots), max_num_annots, 10), dtype='float32') * -1
                for idx, annot in enumerate(transformed_soft_annots):
                    if annot.shape[0] > 0:
                        transformed_annots_padded_soft[idx, :annot.shape[0], :] = annot
            else:
                transformed_annots_padded_soft = np.ones((len(transformed_soft_annots), 1, 10), dtype='float32') * -1
            ...
            if num_fg_crop > 0 and select_fg_crop:
                ...
                if not select_more_soft_fg_crop:
                    ...
                    bg_indices = np.where(valid_mask == 0)[0]
                    num_soft_in_bg = []
                    for idx in bg_indices:
                        transformed_soft_annots_b = transformed_soft_annots[idx]
                        transformed_soft_annots_b = transformed_soft_annots_b[transformed_soft_annots_b[:, -1] != -1]
                        num_soft_in_bg.append(len(transformed_soft_annots_b))
                    num_soft_in_bg = np.array(num_soft_in_bg)
                    # Sort by the number of soft pseudo label
                    bg_indices = bg_indices[np.argsort(num_soft_in_bg)[::-1]]
