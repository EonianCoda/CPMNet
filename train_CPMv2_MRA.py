...
parser.add_argument('--train_csv', type=str, default='train_refine.csv', help='train_list')
parser.add_argument('--val_csv', type=str, default='val.csv', help='val_list')
...
def training_data_prepare(crop_size=CROP_SIZE, blank_side=0):
    ...
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True,
                              collate_fn=collate_fn_dict,num_workers=args.num_workers, pin_memory=False, drop_last=True)
...
def test_val_data_prepare(roots, csv_file):
    ...
    print("Val Data : ", len(test_loader.dataset))
    return test_loader
...
def train(epoch, train_loader, model=None):
    ...
    logger.info('entrance train')
    model.train().to(device)
    logger.info('model warp to device.{}'.format(device))
    total_cls_loss = 0
    total_shape_loss = 0
    total_offset_loss = 0
    total_iou_loss = 0
